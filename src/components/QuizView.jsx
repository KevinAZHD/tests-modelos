...
import { ChevronLeft, ChevronRight, RotateCcw, Search, Home } from 'lucide-react';
...
export const QuizView = ({ title, rawQuestions, onBackToMenu }) => {
  ...
  useEffect(() => {
    const handleBeforeUnload = (e) => {
      if (gameState === 'playing' && Object.keys(answers).length > 0 && !isRevision) {
        e.preventDefault();
  ...
  // Preguntar antes de volver a inicio si está a medias
  const handleBack = () => {
    if (gameState === 'playing' && Object.keys(answers).length < questions.length) {
    ...
    return (
      <div className="min-h-screen bg-white dark:bg-black text-gray-900 dark:text-white flex flex-col items-center justify-center p-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <button onClick={() => { setGameState('playing'); setIsRevision(true); setCurrentIdx(0); }} className="flex items-center justify-center gap-2 bg-gray-200 dark:bg-zinc-800 hover:bg-gray-300 dark:hover:bg-zinc-700 text-black dark:text-white font-bold py-4 px-6 rounded-xl">
          ...
          <button onClick={onBackToMenu} className="flex items-center justify-center gap-2 bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 hover:bg-gray-100 dark:hover:bg-zinc-800 text-black dark:text-white font-bold py-4 px-6 rounded-xl">
            <Home size={24} /> INICIO
          </button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black text-gray-900 dark:text-white p-4 md:p-8 flex flex-col justify-center">
      ...
      <div className="sticky top-0 z-40 bg-white dark:bg-black py-3 md:static md:z-auto md:bg-transparent md:dark:bg-transparent md:py-0 max-w-4xl w-full mx-auto flex flex-col gap-4 mb-6">
